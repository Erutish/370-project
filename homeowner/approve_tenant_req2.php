...
// Check if property_id and action are provided in the URL
if (isset($_GET['property_id']) && isset($_GET['action'])) {
    $tenant_nid = $_GET['tenant_nid'];
    $propertyId = $_GET['property_id'];
    $action = $_GET['action'];

    // Update the status in booking_requests table
    if ($action === 'approve') {
        ...
        $updateAvailabilitySql = "UPDATE room SET availability_status='booked', tenant_nid='$tenant_nid', due_date='$formattedDueDate' WHERE property_id='$propertyId'";
